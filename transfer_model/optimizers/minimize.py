...
def minimize(
    ...
    **kwargs
    ...
    for n in range(maxiters):
        ...
        if n > n_ftol and prev_loss is not None and ftol > 0:
            if is_adam:
                loss_rel_change = abs(rel_change(prev_loss, loss.item()))
            else:
        ...
        if interactive and n % summary_steps == 0:
            logger_stream = f'[{n:05d}] Loss: {loss.item():.4f}'
            if summary_closure is not None:
                summaries = summary_closure()
                for key, val in summaries.items():
                    logger_stream += f' | {key}: {val:.4f}'
            logger.info(logger_stream)
    ...
    # Save the final step
    if interactive:
        logger_stream = f'[{n + 1:05d}] Loss: {loss.item():.4f}'
        if summary_closure is not None:
            summaries = summary_closure()
            for key, val in summaries.items():
                logger_stream += f' | {key}: {val:.4f}'
